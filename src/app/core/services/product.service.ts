...
export class ProductService {
  ...
  registerPurchase(
    ...
    this.http.post<Product>(apiUrl(`/admin/products/${id}/purchases`), input).subscribe({
      next: (p) => {
        this.adminStore.reload();
        this.publicStore.load();
        onSuccess?.(p);
  ...
  /** Genera un código de barras interno (no un EAN real) para imprimir en la etiqueta. */

...
/**
 * Etiqueta imprimible con el código de barras de un producto — no es un EAN
 * real (no hay autoridad emisora), es el código interno generable desde el
 * form de producto (`ProductService.generateBarcode`). Mismo patrón de
 * impresión que `admin-product-qr` (que sigue siendo la opción de siempre,
 * no se reemplaza — un producto puede tener QR, código de barras, o ambos).
 */
@Component({
